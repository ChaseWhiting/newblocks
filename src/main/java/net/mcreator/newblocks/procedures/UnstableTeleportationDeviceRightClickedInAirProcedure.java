...
public class UnstableTeleportationDeviceRightClickedInAirProcedure {
	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		for (int index0 = 0; index0 < (int) (15); index0++) {
			NewBlocksModVariables.WorldVariables.get(world).ReturnXValue = (new Random().nextInt(50 + 1));
			NewBlocksModVariables.WorldVariables.get(world).syncData(world);
			NewBlocksModVariables.WorldVariables.get(world).ReturnZValue = (new Random().nextInt(50 + 1));
			NewBlocksModVariables.WorldVariables.get(world).syncData(world);
			if (world.isAirBlock(new BlockPos(NewBlocksModVariables.WorldVariables.get(world).ReturnXValue,
					world.getHeight(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, (int) NewBlocksModVariables.WorldVariables.get(world).ReturnXValue,
