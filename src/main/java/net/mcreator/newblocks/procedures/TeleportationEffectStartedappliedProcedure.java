...
public class TeleportationEffectStartedappliedProcedure {
	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		NewBlocksModVariables.WorldVariables.get(world).ReturnXValue = (new Random().nextInt((int) (10000 - new Random().nextInt(8000 + 1)) + 1));
		NewBlocksModVariables.WorldVariables.get(world).syncData(world);
		NewBlocksModVariables.WorldVariables.get(world).ReturnZValue = (new Random().nextInt((int) (10000 - new Random().nextInt(8000 + 1)) + 1));
		NewBlocksModVariables.WorldVariables.get(world).syncData(world);
		NewBlocksModVariables.WorldVariables.get(world).ReturnYValue = (new Random().nextInt(55 + 1));
		NewBlocksModVariables.WorldVariables.get(world).syncData(world);
		if (entity instanceof PlayerEntity) {
			new Object() {
				...
				private void run() {
					if ((world.getBlockState(new BlockPos(NewBlocksModVariables.WorldVariables.get(world).ReturnXValue,
							NewBlocksModVariables.WorldVariables.get(world).ReturnYValue + 1,
							NewBlocksModVariables.WorldVariables.get(world).ReturnZValue))).getBlock() == Blocks.CAVE_AIR) {
						{
							Entity _ent = entity;
						...
						{
							Entity _ent = entity;
							_ent.setPositionAndUpdate((new Random().nextInt((int) (10000 - new Random().nextInt(8000 + 1)) + 1)),
									(world.getHeight(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, (int) (entity.getPosX()), (int) (entity.getPosZ()))
											+ 8.5),
									(new Random().nextInt((int) (10000 - new Random().nextInt(8000 + 1)) + 1)));
							if (_ent instanceof ServerPlayerEntity) {
								((ServerPlayerEntity) _ent).connection.setPlayerLocation(
										(new Random().nextInt((int) (10000 - new Random().nextInt(8000 + 1)) + 1)),
										(world.getHeight(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, (int) (entity.getPosX()), (int) (entity.getPosZ()))
												+ 8.5),
										(new Random().nextInt((int) (10000 - new Random().nextInt(8000 + 1)) + 1)), _ent.rotationYaw,
