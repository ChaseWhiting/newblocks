...
import net.mcreator.newblocks.entity.GoldenAllayEntity;

import com.mojang.blaze3d.vertex.IVertexBuilder;
import com.mojang.blaze3d.matrix.MatrixStack;

@OnlyIn(Dist.CLIENT)
public class GoldenAllayRenderer {
	public static class ModelRegisterHandler {
		@SubscribeEvent
		@OnlyIn(Dist.CLIENT)
		public void registerModels(ModelRegistryEvent event) {
			RenderingRegistry.registerEntityRenderingHandler(GoldenAllayEntity.entity, renderManager -> {
				return new MobRenderer(renderManager, new Modelallay(), 0.2f) {

					@Override
					public ResourceLocation getEntityTexture(Entity entity) {
						return new ResourceLocation("new_blocks:textures/golden_allay.png");
					}
				};
			});
		}
	...
	public static class Modelallay extends EntityModel<Entity> {
		...
		public void setRotationAngles(Entity e, float f, float f1, float f2, float f3, float f4) {
			this.right_wing_r1.rotateAngleZ = MathHelper.cos(f * 0.1111F + (float) Math.PI) * f1;
			this.left_wing_r1.rotateAngleZ = MathHelper.cos(f * 0.1111F) * f1;
			this.right_wing_r1.rotateAngleX = MathHelper.cos(f * 0.6662F + (float) Math.PI) * f1;
			this.left_wing_r1.rotateAngleX = MathHelper.cos(f * 0.6662F + (float) Math.PI) * f1;
		}
	}
