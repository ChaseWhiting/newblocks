...
import net.mcreator.newblocks.entity.VultureInSkyEntity;

import com.mojang.blaze3d.vertex.IVertexBuilder;
import com.mojang.blaze3d.matrix.MatrixStack;

@OnlyIn(Dist.CLIENT)
public class VultureInSkyRenderer {
	public static class ModelRegisterHandler {
		@SubscribeEvent
		@OnlyIn(Dist.CLIENT)
		public void registerModels(ModelRegistryEvent event) {
			RenderingRegistry.registerEntityRenderingHandler(VultureInSkyEntity.entity, renderManager -> {
				return new MobRenderer(renderManager, new Modelvulture_in_sky(), 0.4f) {

					@Override
					public ResourceLocation getEntityTexture(Entity entity) {
						return new ResourceLocation("new_blocks:textures/vulture_land.png");
					}
				};
			});
		}
	...
	public static class Modelvulture_in_sky extends EntityModel<Entity> {
		...
		this.left_wing.rotateAngleZ = MathHelper.cos(f * 0.8F) * 1F * f1 +1F;
			this.left_wing.rotateAngleZ = MathHelper.cos(f * 0.8F) * 1F * f1 +1F;
			this.left_wing.rotateAngleZ = MathHelper.cos(f * 0.8F) * 1F * f1 +1F;
			this.right_wing.rotateAngleZ = MathHelper.cos(f * 0.8F + (float) Math.PI) * 1F * f1 -1F;
			this.right_wing.rotateAngleZ = MathHelper.cos(f * 0.8F + (float) Math.PI) * 1F * f1 -1F;
			this.right_wing.rotateAngleZ = MathHelper.cos(f * 0.8F + (float) Math.PI) * 1F * f1 -1F;
		}
	}
